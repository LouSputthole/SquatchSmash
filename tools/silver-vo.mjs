#!/usr/bin/env node
/** Synchronize every Front and Center spoken line into the global manifest. */
...
const expected = allSilverVoiceLines().map((line) => ({
  name: line.name,
  voice: line.voice,
  say: line.text,
}));
const current = manifest.sfx.filter((cue) => cue.name.startsWith('vo.silver.'));
...
if (checkOnly) {
  if (missing.length || stale.length || drifted.length) {
    console.error(`Silver VO drift: ${missing.length} missing, ${stale.length} stale, ${drifted.length} changed.`);
    process.exit(1);
  }
  console.log(`${expected.length} Silver voice cues match the authored catalog.`);
  process.exit(0);
}

const kept = manifest.sfx.filter((cue) => !cue.name.startsWith('vo.silver.'));
const insertAt = kept.findIndex((cue) => cue.name.startsWith('vo.beefrun.'));
...
const byVoice = {};
for (const cue of expected) byVoice[cue.voice] = (byVoice[cue.voice] ?? 0) + 1;
console.log(`${expected.length} Silver voice cue(s) written.`);
