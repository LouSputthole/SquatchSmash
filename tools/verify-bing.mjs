...
const dialogueContracts = await page.evaluate(() => {
  ...
  const louCue = typeof louEnter.cue === 'function' ? louEnter.cue() : louEnter.cue;
  const snowReply = b.familyScripts.snow.open.options[0];
  return {
    louOpening: typeof louEnter.line === 'function' ? louEnter.line() : louEnter.line,
    louCue,
    louDuration: b.audio.buffers.get(louCue)?.[0]?.duration ?? 0,
    snowCue: snowReply.cue,
    snowText: snowReply.text,
  };
});
check('Lou opens the office conversation with his own clean recorded line',
  dialogueContracts.louOpening === 'Shut the door.'
    && dialogueContracts.louCue === 'vo.bing.full.lou.enter.line.1oe6npq'
    && dialogueContracts.louDuration > 0,
...
const clocksAndPowder = await page.evaluate(() => {
  ...
  return {
    ...
    focus,
    fovBefore,
    consumed: !b.club.anchors.powderMesh?.parent,
  };
});
