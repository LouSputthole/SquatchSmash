...
try {
  ...
  const panel = await page.evaluate(() => {
    ...
    /* The card is persistent now (owner, 2026-08-31: objectives "display then
     * basically dissapear" -- the twelve-second fade defaults off), but
     * `setObjectives` still only touches the DOM when the list reads
     * differently, so reveal explicitly rather than depend on what an earlier
     * draw left behind. This check is about whether required and optional
     * rows are drawn differently, not about when the panel happens to be up. */
