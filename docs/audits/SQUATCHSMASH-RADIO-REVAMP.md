...
| 4 | Add stop, restore, overlap, and teardown browser receipts | tools/radio-active-play-coverage.mjs; existing Playwright scene verifiers; audio residency tests | Stable receiver IDs and current music ownership | Medium | Every unique non-silent timeline owner maps to an exact named active-play receipt; contract rejects missing/stale mappings and renamed checks | SOURCE COMPLETE — 26/26 OWNERS MAPPED; BROWSER RERUN DUE |  |
...
| 9 | Regenerate ledgers and certify the final radio revamp | Generated dialogue/audio/take ledgers; check:radio-vo; scene tests; campaign marathon if handoffs change | Any authored line or route changes | Low | All applicable cue, take, audio, radio, scene, and campaign gates actually run and pass | READY — NAMED BROWSER RECEIPTS + FULL GATES PENDING |  |
