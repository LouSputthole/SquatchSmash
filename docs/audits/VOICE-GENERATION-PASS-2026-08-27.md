...
- The final full-history audit caught **1** additional Enola take that predated the wrong-city-clue rewrite. Irish no longer speaks the city name; the replacement keeps that clue exclusively on the cockpit instrument, as authored.
...
- [rendered-voice-receipts.json](./voice/rendered-voice-receipts.json): **387 / 387** exact rendered takes, including all 207 backlog lines, the three audited announcer replacements, and the final corrected Irish take, match the current cue text hash and voice ID and decode in the repository's Playwright Chromium. Total decoded duration is 1,170.125 seconds; the new backlog batch accounts for 553.786 seconds.
