...
- The three-click swing now recommends a useful power for the club, lie and
  ...
  controllable fade that compounds into a slice on an early strike. The
  second strike sweep is deliberately slower than the power sweep so it can
  be read and timed instead of guessed.
- Every golfer now addresses from beside the ball with shoulders perpendicular
  to the target line. The browser round verifies this against all three authored
  NPC tee targets instead of accepting a character who faces the pin.
...
- Addressing draws a yellow projected landing circle and distance label from
  the same club, live power, lie and aim values used by the shot estimate. Its
  radius communicates dispersion, so it previews a landing area rather than
  promising an exact aim-bot point.
...
- `10-ball-finder.png` verifies the physical ring, edge waypoint and top map.
  ...
  same map while the cart is moving. The camera now faces the cart's actual
  travel direction, and the physical dashboard receiver, green power lamp and
  shared 97.8 station read are visible; R powers it and T/N tune it.
- `12-shot-result.png` verifies the post-shot result card after a real
  ...
  positions.
- `13-landing-preview.png` verifies the projected yellow landing area and its
  truthful live yardage before the first click.
...
- Completed holes, penalties, the invitation, the cart conversation, the NPC
  ...
  unfinished tee; direct standalone loads remain blocked without mutating
  campaign state.
...
- `13-landing-preview.png`: projected yellow landing area and truthful live
  yardage before the first swing click.
...
4. **Map detail:** the world view now owns a live landing-area preview, while the
   map proves player, ball, pin, carts and course route. The map should still add
   hazard fills, a wind arrow and its own compact selected-club range overlay.
