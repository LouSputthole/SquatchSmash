...
| Mansion / Silent Squatch | Fixed and browser-green | 273/273 in 481.7s. The expanded real-input run passes the walkable pool entry/basin, approved crests on both gate faces, guest-room crest/photo correction, birdcage tray, physically identified Booski/DeathMegatron bedroom, and returning Bada Bing grey sedan. It reports 27 visible lights and 649 shadow casters. Draw calls at the two recorded poses are now governed by the 2026-08-19 room/portal visibility pass (`MansionInterior.js` ROOM / PORTAL VISIBILITY: per-room groups, a precomputed portal graph, three hops, stairs cost two; walls, stairs, lights and instanced batches stay global; `?novis=1` kill switch; culling is visibility-only — hidden rooms still tick): measured headless on the current build, 14,424 gate-view and 8,061 foyer-view draw calls with the pass on, against 15,984 and 10,024 with it off (the previously recorded 14,657/9,744 predate the rooms the house has since grown). Twenty-three spoken recordings and 51 sampled effects remain external production pickups. |
