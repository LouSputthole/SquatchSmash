...
| A verifier left behind by a rewrite | `verify:initiation` was written for the OLD gauntlet Initiation, never updated for the cabin ceremony, and threw a TypeError at line 113 — so five of the scene's six acts had never been checked once, and its whole fifth act shipped broken | **A rewrite is not finished until its verifier is rewritten too.** Scene and check are one change. A check that still boots is not a check that still checks |
