...
| Parent the foyer centrepiece into one named group | It is 70 loose meshes and one collider, parented flat to the interior root; the siege has to find it by sweeping every child within 1.5 m of the anchor and above the floor inlay | An anchor says where it is; a group is what you can actually hide. The sweep works and degrades gracefully, but it also catches anything else that lands in that footprint | Base, Siege | no | no | no | high | low | now, cheap |
| Re-plan the living room's west glazing, or the `living_west` staging | The west wing was hung off that elevation, so `livingWest` (z 47.6–50.8) now looks from the living room into the trophy hall, not outdoors. PART VII stages wave 2B on "the west living room's windows", which an attacker cannot reach from the grounds | Wave 2B's entry has to be a real way in. The siege currently breaks the wing's own glazing first and treats the living room's as a second pane | Base, Siege | yes | no | no | med | med | with the overview |
| Split `livingWestNorth` at the wing partition | The pane runs z 54.4–57.2 and the trophy hall stops at 55.7 while the winter garden starts at 56.0, so one 2.8 m window looks into two rooms and, for 0.3 m of its width, straight at the partition between them | A window whose middle is a wall reads as a modelling error from either side | Base | no | no | yes | low | low | now, cheap |
