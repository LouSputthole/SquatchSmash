...
cues**, generated by the ordinary `npm run sfx -- --voice-only` batch. The
one exception: Numbskull's two lines below stay out of the manifest until
his voice id lands (an unset id aborts a generation run); add them with the
id.
