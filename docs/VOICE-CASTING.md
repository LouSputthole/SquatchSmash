...
shot, blackjack VO). These enter `assets/sfx/manifest.json` as spoken cues
right after the in-flight Bada Bing scene pass lands (it owns the manifest's
sfx block this hour); the scripts here are the authored copy.
