...
cue('golf.h1.rippin.eric_not_cricket', RIPPIN,
...
cue('golf.h2.rippin.eric_finds_it_every_time', RIPPIN,
...
/* ================================================================== */
/* PROSPECT REPLY CHOICES                                              */
/* ================================================================== */

/* These are selected dialogue performances, not UI labels. Keeping them in
 * the same cue registry as every other spoken line gives each exact reply one
 * recording filename and lets the option display read from that same text. */
cue('golf.h1.prospect.arrival_on_time', PROSPECT,
  'You said eight. It’s eight.',
  { direction: 'Dry and matter-of-fact. He followed the instruction exactly.', priority: PRIORITY.STORY, once: true, interruptible: false });
cue('golf.h1.prospect.arrival_thought_work', PROSPECT,
  'I thought this was work.',
  { direction: 'Suspicious, but not complaining. He is trying to read the assignment.', priority: PRIORITY.STORY, once: true, interruptible: false });
cue('golf.h1.prospect.arrival_better_shoes', PROSPECT,
  'I own better shoes.',
  { direction: 'A small defensive correction delivered too late to help.', priority: PRIORITY.STORY, once: true, interruptible: false });

cue('golf.h1.prospect.needed_a_fourth', PROSPECT,
  'You needed a fourth.',
...
cue('golf.h1.prospect.earned_morning_off', PROSPECT,
  ...
  { direction: 'Confident without swagger. Testing whether he is allowed to say it.', priority: PRIORITY.STORY, once: true, interruptible: false });
cue('golf.h1.prospect.whats_the_catch', PROSPECT,
  'What’s the catch?',
...
export const SEQUENCES = Object.freeze({
  ...
  'tee.eric.before': ['golf.h1.rippin.eric_not_cricket', 'golf.h1.eric.cricket_rules'],
  'tee.eric.after': [
  ...
  'h2.tee.eric.before': ['golf.h2.rippin.eric_finds_it_every_time'],
...
/**
 * The four places the player answers rather than listens.
 *
...
export function buildScripts(ctx) {
  ...
  const reply = (cueId, option) => ({
    ...option,
    cue: cueId,
    text: CUES[cueId].text,
  });
  ...
  const arrival = {
    ...
    answer: {
      ...
      line: null,
      options: [
        reply('golf.h1.prospect.arrival_on_time', {
          tone: 'Direct',
          next: 'direct',
          effect: () => ctx.remember('arrival.direct'),
        }),
        reply('golf.h1.prospect.arrival_thought_work', {
          tone: 'Suspicious',
          next: 'suspicious',
          effect: () => ctx.remember('arrival.suspicious'),
        }),
        reply('golf.h1.prospect.arrival_better_shoes', {
          tone: 'Defensive',
          next: 'defensive',
  ...
  const firstTee = {
    ...
    answer: {
      ...
      line: null,
      options: [
        reply('golf.h1.prospect.needed_a_fourth', {
          tone: 'Modest',
          next: 'modest',
          effect: () => { ctx.remember('tee.modest'); ctx.flag('heardInvitation'); },
        }),
        reply('golf.h1.prospect.another_test', {
          tone: 'Suspicious',
          next: 'test',
          effect: () => ctx.remember('tee.test'),
        }),
        reply('golf.h1.prospect.earned_morning_off', {
          tone: 'Confident',
          next: 'confident',
          effect: () => ctx.remember('tee.confident'),
        }),
        reply('golf.h1.prospect.whats_the_catch', {
          tone: 'Joking',
          next: 'catch',
  ...
  const cartRide = {
    ...
    answer: {
      ...
      options: [
        ...
        reply('golf.h1.prospect.got_lucky', {
          tone: 'Modest',
          next: 'modest',
  ...
  if (trees) {
    const walk = (node) => {
      ...
      if (Array.isArray(node.options)) {
        for (const option of node.options) if (option.cue) reachable.add(option.cue);
      }
    };
