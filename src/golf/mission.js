...
export class Round {
  ...
  _updateNpcApproachJob(job, dt) {
    ...
    if (job.phase === 'finish') {
      ...
      ball.placeAt(HOLE.pin.x, HOLE.pin.z);
      ball.state = BALL_STATE.HOLED;
      this.card.finish(job.id, HOLE.number);
      golfer.retrieveFromCup();
      this.audio?.holed({ ...ball.position });
      this._pendingHoleOut?.delete(job.id);
      return true;
    }
   ...
   *
   * An NPC whose authored last stroke lipped out still walks to the cup and
   * retrieves it. The authored final stroke is already on the card at impact;
   * adding a phantom tap-in here would corrupt the scores the dialogue uses.
   */
  _finishStragglers() {
    for (const id of [ERIC, LOU, RIPPIN]) {
      if (this.card.finished(id, HOLE.number)) continue;
      ...
      if (ball.state === BALL_STATE.HOLED || this._pendingHoleOut?.has(id)) {
        ...
        const golfer = this.golfers[id];
        if (!golfer) continue;
        const dx = golfer.position.x - HOLE.pin.x;
        const dz = golfer.position.z - HOLE.pin.z;
        const length = Math.hypot(dx, dz) || 1;
        golfer.walkTo(
          HOLE.pin.x + (dx / length) * 0.62,
          HOLE.pin.z + (dz / length) * 0.62,
          { speed: 1.65 },
        );
        this._npcApproachJobs.set(id, {
          id,
