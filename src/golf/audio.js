...
/** Play the exact cue authored on a selected Prospect dialogue option. */
export function playRecordedGolfChoice(engine, option, opts = {}) {
  const cueId = typeof option?.cue === 'function' ? option.cue() : option?.cue;
  return cueId ? playRecordedGolfCue(engine, cueId, opts) : null;
}
...
function moveLoopPanner(handle, position, now = 0) {
  const panner = handle?.panner;
...
export class CourseAudio {
  ...
  footstep(surface, intensity = 1, position = null) {
    ...
    this.engine?.play(step === 'water' ? 'footstep.puddle' : `footstep.${step}`, {
  ...
  cartMotor(on, position = null) {
    ...
    if (on && !this._cartRunning) {
      ...
      this._cartLoop = this.engine.startLoop('cart.motor', { volume: 0.7, position });
    } else if (!on && this._cartRunning) {
      this._cartRunning = false;
      this.engine.stopLoop('cart.motor', 0.6);
      this._cartLoop = null;
    }
    if (on) moveLoopPanner(this._cartLoop, position, this.engine.ctx?.currentTime ?? 0);
  }
