...
export function createAttackerPool({
    ...
    entry.aimFrame = entry.weaponAim.update(dt, {
      ...
      pose: (frame) => {
        ...
        entry.figure.parts.head.rotation.y = frame.hasTarget
          ...
          : 0;
        entry.figure.parts.armR.rotation.set(-1.28 - frame.pitch, 0, 0.16);
        entry.figure.parts.armL.rotation.set(-1.2 - frame.pitch, 0, -0.34);
        ...
        if (frame.interrupted) {
          ...
          entry.figure.parts.armR.rotation.x += reaction * 0.48;
          entry.figure.parts.armR.rotation.z += reaction * 0.2;
  ...
  function spawn(rawOrder, { silent = false } = {}) {
    ...
    entry.figure.parts.head.rotation.set(0, 0, 0);
    if (entry.gun) entry.gun.rotation.x = -Math.PI / 2;
