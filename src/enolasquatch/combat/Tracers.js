 ...
 * couple of hundred streaks in flight at the peak. Built the obvious way, as a
 * mesh per round, that is a couple of hundred draw calls for something the
 * player perceives as texture.
 *
 * So they are one `THREE.InstancedMesh` of a unit box, stretched along Z per
 * instance and pointed down its own flight path. Colour is per-instance, which
 * is what lets the ground batteries fire yellow, the fighters fire white-hot
 * and the turret fire green without a second mesh.
 *
 ...
 * That is the same register `Defense.js` already used ("a scripted roll against
 * a travelling streak's own miss distance") and it keeps the hit model in one
 * place per shooter instead of spread across the VFX.
 */
import * as THREE from 'three';
import { boxGeo, clamp } from '../../beefrun/util.js';
...
export class TracerPool {
  ...
  update(dt) {
    ...
    for (let i = 0; i < this.capacity; i++) {
      ...
      const len = Math.max(6, _end.distanceTo(_dir));
