...
startBtn.addEventListener('click', async () => {
  ...
  await audio.init();
  await carRadioReady;
  /* BingAudioEngine already narrows the resident bank to this scene. Calling
   * the shared engine's broad prefix loader first bypasses that ownership plan
   * and decodes unrelated campaign/radio audio, so let the scene adapter own
   * the one startup load. */
  const sfx = await audio.loadManifest();
  console.info(`[sfx] ${sfx.loaded}/${sfx.total} Bing samples loaded; the rest are synthesised.`);
