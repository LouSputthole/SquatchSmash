 ...
 * view-models and muzzle effects can share one convention. The separate slide,
 * frame, barrel, sights, controls, trigger guard and magazine base keep it
 * readable as a modern pistol rather than the two-box placeholder older scenes
 * used for every handgun.
 */
export function makeNineMillimeterPistol(M, { x, y, z, rotY = 0 }) {
  ...
  // Front and rear sights sit above the slide.
  ...
  // Polymer frame, accessory rail and takedown controls.
  ...
  g.add(cylinder({ r: 0.0045, h: 0.038, pos: [0, 0.020, 0.004], rotZ: Math.PI / 2, mat: slideDark }));

  // Angled grip with two panels and a magazine floor plate.
  const grip = new THREE.Group();
  ...
  // Curved trigger and a full guard beneath the frame.
  const trigger = box({ size: [0.006, 0.022, 0.007], pos: [0, -0.010, 0.000], mat: slideDark, rotX: 0.32 });
  g.add(trigger);
  ...
  // Rear slide serrations on both faces.
  for (const sx of [-1, 1]) {
    for (let i = 0; i < 4; i++) {
      g.add(box({
  ...
  return {
    group: g,
    trigger,
    /** Local-space muzzle shared by view-model and world-space effects. */
