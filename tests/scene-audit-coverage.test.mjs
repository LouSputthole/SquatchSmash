...
test('default scene audit inventory matches every non-frozen preview launcher runtime', () => {
  ...
  /* One exclusion. `scene:initiation` is FROZEN -- no extractable headless
   * builder at all, so there is nothing for the audit to mount.
   *
   * `scene:special-meeting` used to be excluded here too, on the grounds that
   * it built headlessly but had never had a geometry review: the moment it
   * became checkable it produced 1,420 findings. That pass has since landed
   * (three states registered in tools/geometry-scenes.mjs, the forest floor
   * and the two cars annotated where they are built, three real defects fixed
   * -- a bench through a telegraph pole, two bins inside each other, and a
   * tree clearance test that compared stumps instead of trunks -- and 57
   * authored collider joins waived pair by pair), so the exclusion is gone and
   * the scene is audited like everything else. */
