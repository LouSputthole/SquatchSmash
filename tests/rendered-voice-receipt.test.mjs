...
test('every exact rendered take has a current text, performer, index, hash, and browser-decode receipt', async () => {
   ...
   * with their takes (he is not on the job; Snow is), so 543. On 2026-09-09
   * the owner put the Scarface line back at the top of the siege stairs
   * ("what happened to the say hello to my little friend line"), so
   * vo.siege.prospect.little_friend's take -- rendered from the retired
   * punch-up wording -- left current evidence for the rerecord queue: 542. */
  ...
  /* Twenty-four outstanding, each with a ledger entry the booth sheet shows.
   * Missing: vo.wake.5 (retired with the five-PM start), the twelve THE
   ...
   * rooms they were quiet in -- and the eight Booski silver-case call lines
   * that reached the manifest on 2026-09-09, when the owner heard the
   * luxury-apartment call playing silent ("the voicelines dont wrok for
   * booski"). Re-record: the Squatchfather's Booski line (back to the
   * owner's colostomy-bag pick), the apartment's heist door refusal, which
   * still names the Bing, and the siege's little-friend payoff, whose take
   * on disk still speaks the retired 2026-08-28 punch-up wording. */
  assert.equal(coverage.outstanding, 24);
  assert.equal(coverage.missing.length, 21);
  assert.equal(coverage.rerecord.length, 3);
