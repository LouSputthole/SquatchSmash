...
test('every long-form music master has one current hash-bound loudness receipt', async () => {
  ...
  assert.equal(evidence.measurements.length, 24);
...
test('the generated radio inventory publishes measurements without claiming normalization', async () => {
  ...
  assert.equal(musicRows.length, 24);
  assert.equal(musicRows.filter((row) => String(row['Loudness issue']).startsWith('Measured:')).length, 24);
  assert.equal(data.summary.measuredMasters, 24);
  assert.equal(data.summary.musicMasters, 24);

  const loudnessFinding = data.rows['Problems and Decisions']
    .find((row) => row['Station or cue'] === 'Mix / loudness');
  assert.match(loudnessFinding.Problem, /24\/24/);
